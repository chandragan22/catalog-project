...
def listItemJSON(category_id, list_id):
    List_Item = session.query(CategoryItem).filter_by(id=list_id).one()
...
def newListItem(category_id):
    if 'username' not in login_session:
        ...
        if login_session['user_id'] != category.user_id:
            return "<script>function myFunction() \
            {alert('You are not authorized to add list items to this category.\
...
@app.route(
            '/category/<int:category_id>/list/<int:list_id>/edit',
            methods=['GET', 'POST'])
def editListItem(category_id, list_id):
    if 'username' not in login_session:
        return redirect('/login')
    editedItem = session.query(
        CategoryItem).filter_by(id=list_id).one()
...
# Delete a list item
@app.route(
            '/category/<int:category_id>/list/<int:list_id>/delete',
            methods=['GET', 'POST'])
def deleteListItem(category_id, list_id):
    if 'username' not in login_session:
        return redirect('/login')
    category = session.query(Category).filter_by(id=category_id).one()
    itemToDelete = session.query(CategoryItem).filter_by(id=list_id).one()
    if login_session['user_id'] != category.user_id:
        return "<script>function myFunction() \
